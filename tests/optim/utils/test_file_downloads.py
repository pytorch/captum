...
class TestFileExistence(unittest.TestCase):
    def test_clip_bpe_simple_vocab_48895_txt(self) -> None:
        url = BASE_UR_MODELSL + "clip_bpe_simple_vocab_48895.txt"
    ...
    def test_inceptionv1_mixed4c_relu_samples_activations_pt(self) -> None:
        ...
        state_dict = torch.hub.load_state_dict_from_url(url, progress=False, check_hash=False)
        self.assertTrue(isinstance(state_dict, dict), f"Failed to load state dict from: {url}")
    ...
    def test_inception5h_pth(self) -> None:
        url = BASE_UR_MODELSL + "inception5h.pth"
        state_dict = torch.hub.load_state_dict_from_url(url, progress=False, check_hash=False)
        self.assertTrue(isinstance(state_dict, dict), f"Failed to load state dict from: {url}")
    ...
    def test_clip_resnet50x4_facets_pt(self) -> None:
        url = BASE_UR_MODELSL + "clip_resnet50x4_facets.pt"
        state_dict = torch.hub.load_state_dict_from_url(url, progress=False, check_hash=False)
        self.assertTrue(isinstance(state_dict, dict), f"Failed to load state dict from: {url}")

    def test_clip_resnet50x4_image_pt(self) -> None:
        url = BASE_UR_MODELSL + "clip_resnet50x4_image.pt"
        state_dict = torch.hub.load_state_dict_from_url(url, progress=False, check_hash=False)
        self.assertTrue(isinstance(state_dict, dict), f"Failed to load state dict from: {url}")

    def test_clip_resnet50x4_text_pt(self) -> None:
        url = BASE_UR_MODELSL + "clip_resnet50x4_text.pt"
        state_dict = torch.hub.load_state_dict_from_url(url, progress=False, check_hash=False)
        self.assertTrue(isinstance(state_dict, dict), f"Failed to load state dict from: {url}")
