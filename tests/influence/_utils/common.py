...
def get_random_model_and_data(
    ...
    The need to differentiate between `'cuda'` and `'cuda_data_parallel'` is
    that sometimes we may want to test a model that is on cpu, but is *not* wrapped
    in `DataParallel`.
    """
...
def generate_symmetric_matrix_given_eigenvalues(eigenvalues: List[float]):
    ...
    return torch.matmul(Q, torch.matmul(torch.diag(eigenvalues), Q.T))
...
def generate_assymetric_matrix_given_eigenvalues(eigenvalues: List[float]):
    ...
    D = len(eigenvalues)
    Q_T = torch.randn((D, D))
    return torch.linalg.solve(Q_T, torch.matmul(torch.diag(eigenvalues), Q_T)).T
