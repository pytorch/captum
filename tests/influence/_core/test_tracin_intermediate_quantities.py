...
class TestTracInIntermediateQuantities(BaseTest):
    ...
    def test_tracin_intermediate_quantities_projection_consistency(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            """
            net has in_features, hidden_nodes (layer input), out_features (jacobian) = 5, 4, 3
            and 5 checkpoints

            projection only happens
            (A) if project_dim < input_dim * output_dim of final layer  ( 4 * 3 = 12 here )
            output_dim is referred to as jacobian_dim and input_dim as layer_input in code.

            if jacobian_dim < int(sqrt(projection dim)), then it is not projected down
            if layer_input_dim < int(sqrt(projection dim)), then it is not projected down
            ...
            (B) final_projection_dim = jacobian_dim_post * layer_input_dim_post * num_checkpoints
