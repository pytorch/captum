    ...
    def __new__(cls, name, bases, attrs):
        for test_config in config:
            ...
            for algorithm in algorithms:
                ...
                test_method = cls.make_single_target_test(algorithm, model, layer, args, target_delta, noise_tunnel, baseline_distr)
                test_name = "test_target_" + test_config["name"] + "_" + algorithm.__name__ + ("NoiseTunnel" if noise_tunnel else "")
                attrs[test_name] = test_method
        return super(TargetsMeta, cls).__new__(cls, name, bases, attrs)

    @classmethod
    @deep_copy_args
    def make_single_target_test(cls, algorithm, model, layer, args, target_delta, noise_tunnel, baseline_distr):
        """
        ...
        original_additional_forward_args = _format_additional_forward_args(args["additional_forward_args"]) if "additional_forward_args" in args else None
        num_examples = (
            len(original_inputs)
        ...
        def target_test_assert(self):
            ...
            for i in range(num_examples):
                ...
                if original_additional_forward_args is not None:
                    args["additional_forward_args"] = tuple(single_add_arg[i : i + 1] if isinstance(single_add_arg, Tensor) else single_add_arg for single_add_arg in original_additional_forward_args)
                if replace_baselines:
                    if isinstance(original_inputs, Tensor):
            ...
            #if original_additional_forward_args is not None:
            #    args["additional_forward_args"] = original_additional_forward_args
            #if replace_baselines:
