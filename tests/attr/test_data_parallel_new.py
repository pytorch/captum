...
from captum.attr._core.neuron.neuron_guided_backprop_deconvnet import NeuronDeconvolution, NeuronGuidedBackprop
...
class DataParallelMeta(type):
    def __new__(cls, name, bases, attrs):
        for test_config in config:
            ...
            target_delta = test_config["target_delta"] if "target_delta" in test_config else 0.0002
            ...
            for algorithm in algorithms:
                for mode in DataParallelCompareMode:
                    ...
                    test_method = cls.make_single_dp_test(algorithm, model, target_layer, args, target_delta, noise_tunnel, baseline_distr, mode)
                    test_name = "test_" + test_config["name"] + "_" + mode.name + "_" + algorithm.__name__ + ("NoiseTunnel" if noise_tunnel else "")
                    attrs[test_name] = test_method

        return super(DataParallelMeta, cls).__new__(cls, name, bases, attrs)

    @classmethod
    def make_single_dp_test(cls, algorithm, model, target_layer, args, target_delta, noise_tunnel, baseline_distr, mode):
        """
        ...
        if mode is DataParallelCompareMode.cpu_cuda:
            ...
            model_1, model_2 = cuda_model, torch.nn.parallel.DataParallel(cuda_model, device_ids=alt_device_ids)
            args_1, args_2 = cuda_args, cuda_args
        else:
        ...
        def data_parallel_test_assert(self):
            if target_layer:
                attr_method_1 = algorithm(model_1, get_nested_attr(model_1, target_layer))
                ...
                if alt_device_ids is None:
                    attr_method_2 = algorithm(model_2, get_nested_attr(cuda_model, target_layer))
                else:
                    ...
                    if issubclass(algorithm, (LayerDeepLift, LayerDeepLiftShap, NeuronDeconvolution, NeuronGuidedBackprop)):
                        attr_method_2 = algorithm(model_2, get_nested_attr(cuda_model, target_layer))
                    else:
                        attr_method_2 = algorithm(model_2.forward, get_nested_attr(cuda_model, target_layer), device_ids=alt_device_ids)
            else:
            ...
            if attr_method_1.has_convergence_delta():
                attributions_1, delta_1 = attr_method_1.attribute(return_convergence_delta=True, **args_1)
                self.setUp()
                attributions_2, delta_2 = attr_method_2.attribute(return_convergence_delta=True, **args_2)
                assertTensorTuplesAlmostEqual(
                    self, attributions_1, attributions_2, delta=target_delta
                )
                assertTensorTuplesAlmostEqual(
                    self, delta_1, delta_2, delta=target_delta
                )
            else:
