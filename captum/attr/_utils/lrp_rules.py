...
class WSquaredRule(PropagationRule):
    ...
    def forward_hook(self, module: torch.nn.Module, inputs: Tuple[Tensor, ...], outputs: Tensor) -> None:
        super().forward_hook(module, inputs, outputs)
        self._compute_denominator(module, inputs)

    def _compute_denominator(self, module: torch.nn.Module, inputs: Tuple[Tensor, ...]) -> None:
        ...
        if input_shapes != self._input_shapes:
            ...
            with torch.no_grad():
                self._denominator = module.forward(
                    *tuple(torch.ones(input_shape).unsqueeze(dim=0) for input_shape in self._input_shapes)
                )
                self._denominator += self.STABILITY_FACTOR

    def _create_backward_hook_input(self, input_: Tensor, input_index: int) -> Callable[[Tensor], None]:
        def _backward_hook_input(grad: Tensor) -> None:  # pylint: disable=unused-argument
            pass

        return _backward_hook_input

    def _create_backward_hook_output(self, output: Tensor) -> Callable[[Tensor], Tensor]:
        def _backward_hook_output(grad: Tensor) -> Tensor:
            relevance = grad / torch.cat(tuple(self._denominator for _ in range(output.shape[0])))
            return relevance

        return _backward_hook_output

    def _manipulate_weights(self, module: torch.nn.Module, inputs: Tuple[Tensor, ...], outputs: Tensor) -> None:
...
class FlatRule(WSquaredRule):
    ...
    def _manipulate_weights(self, module: torch.nn.Module, inputs: Tuple[Tensor, ...], outputs: Tensor) -> None:
        if hasattr(module, "bias"):
            if module.bias is not None:
                module.bias.data = torch.zeros_like(module.bias.data)  # pylint: disable=E1101
        if hasattr(module, "weight"):
            module.weight.data = torch.ones_like(module.weight.data)  # pylint: disable=E1101
...
class AlphaBetaRule(PropagationRule):
    ...
    def _create_backward_hook_output(self, output: Tensor) -> Callable[[Tensor], Tensor]:
        def _backward_hook_output(grad: Tensor) -> Tensor:
            out = self._compute_signed_contributions(grad, self.inputs_pos, self.inputs_neg, +1)

            if self.beta:
                out_beta = tuple(
                    self.beta * rel
                    for rel in self._compute_signed_contributions(grad, self.inputs_neg, self.inputs_pos, -1)
                )
    ...
    def _compute_signed_contributions(
        self, grad: Tensor, mod_pos_in: Tuple[Tensor, ...], mod_neg_in: Tuple[Tensor, ...], sign: int
    ) -> Tuple[Tensor, ...]:
        r"""
        if mod_pos_in is the positive part of the inputs and mod_neg_in the negative this computes the alpha part
        if mod_pos_in is the negative part of the inputs and mod_neg_in the positive this computes the beta part
        """
        with torch.autograd.set_grad_enabled(True):
            ...
            if not self.set_bias_to_zero and self._bias_contrib is not None:
                if sign == 1:
                    denominator += torch.cat(tuple(self._bias_contrib for _ in range(denominator.shape[0]))).clamp(
                        min=0
                    )
                else:
                    denominator += torch.cat(tuple(self._bias_contrib for _ in range(denominator.shape[0]))).clamp(
            ...
            res_wp = torch.autograd.grad(outputs=mod_pos_out, inputs=mod_pos_in, grad_outputs=rescaled_relevance)
            res_wm = torch.autograd.grad(outputs=mod_neg_out, inputs=mod_neg_in, grad_outputs=rescaled_relevance)

            out = tuple(
                (pos_in * jac_pos) + (neg_in * jac_neg)
                for jac_pos, pos_in, jac_neg, neg_in in zip(res_wp, mod_pos_in, res_wm, mod_neg_in)
            )
    ...
    def _separate_weights_by_sign(self, module: torch.nn.Module) -> None:
                ...
                if not self.set_bias_to_zero and self._bias_contrib is None:
                    with torch.no_grad():
                        self._bias_contrib = module.forward(
                            *(torch.zeros(input_.shape[1:]).unsqueeze(dim=0) for input_ in inputs)
                        )
                module.bias.data = torch.zeros_like(module.bias.data)
...
class AlphaBetaRuleV2(AlphaBetaRule):
        ...
        def _backward_hook_output(grad: Tensor) -> Tensor:
            ...
            if self.beta:
                ...
                if self.set_bias_to_zero and self._bias_contrib is not None:
                    neg_denominator -= torch.cat(tuple(self._bias_contrib for _ in range(neg_denominator.shape[0])))

                neg_denominator -= self.STABILITY_FACTOR
                beta_rel = (self.beta * grad) / neg_denominator

                res_wp_beta = torch.autograd.grad(
                    inputs=self.inputs_pos, outputs=self.wp_xp, grad_outputs=beta_rel, retain_graph=False
                )
                res_wm_beta = torch.autograd.grad(
                    inputs=self.inputs_neg, outputs=self.wm_xm, grad_outputs=beta_rel, retain_graph=True
                )

                out_beta = list(
                    xm * wp_beta + xp * wm_beta
                    for xm, wp_beta, xp, wm_beta in zip(self.inputs_neg, res_wp_beta, self.inputs_pos, res_wm_beta)
                )
            ...
            res_wm_alpha = torch.autograd.grad(
                inputs=self.inputs_neg, outputs=self.wm_xm, grad_outputs=alpha_rel, retain_graph=False
            )

            out_alpha = list(xm * wm_alpha for xm, wm_alpha in zip(self.inputs_neg, res_wm_alpha))
    ...
    def _compute_pos_denominator(self, output: Tensor) -> Tensor:
        ...
        if not self.set_bias_to_zero and self._bias_contrib is not None:
            pos_denominator += torch.cat(tuple(self._bias_contrib for _ in range(pos_denominator.shape[0]))).clamp(
                min=0
            )
    ...
    def forward_pre_hook_activations(self, module: torch.nn.Module, inputs: Tuple[Tensor, ...]) -> Tuple[Tensor, ...]:
...
class ZBoundRule(PropagationRule):
    def __init__(
        ...
        self._lower_bound_tensor = None
        self._upper_bound_tensor = None

        self._module_pos = None
        self._module_neg = None
        self._bias_contrib = None

        self._denominator_bound_contribution = None
        self._input_shapes = None
    ...
    def _create_backward_hook_input(
        ...
        def _backward_hook_input(grad: Tensor) -> Tensor:
            ...
            relevance += (input_ - self._upper_bound_tensor[input_index]) * self.res_wm[input_index]
    ...
    def _create_backward_hook_output(
        ...
        def _backward_hook_output(grad: Tensor) -> Tensor:
            ...
            if self.set_bias_to_zero and self._bias_contrib is not None:
                denominator -= torch.cat(tuple(self._bias_contrib for _ in range(output.shape[0])))
        ...
        if input_shapes != self._input_shapes:
            ...
            with torch.autograd.set_grad_enabled(True):
                self._upper_bound_tensor = tuple(
                    torch.full(input_shape, self.upper_bound, requires_grad=True) for input_shape in self._input_shapes
                )
                self.upper_bound_contrib = self._module_neg.forward(*self._upper_bound_tensor)

            with torch.no_grad():
                self._lower_bound_tensor = tuple(
                    torch.full(input_shape, self.lower_bound) for input_shape in self._input_shapes
                )
                self._denominator_bound_contribution = (
                    self._module_pos.forward(*self._lower_bound_tensor) + self.upper_bound_contrib
                )
