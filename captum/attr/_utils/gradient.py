...
def compute_gradients(
    ...
    with torch.autograd.set_grad_enabled(True):
        ...
        assert len(output.shape) < 2 or (len(output.shape) == 2 and len(output[0]) == 1), "Target not provided when necessary, cannot take gradient with respect to multiple outputs."
...
def compute_layer_gradients_and_eval(
    ...
    with torch.autograd.set_grad_enabled(True):
        ...
        hook = layer.register_forward_hook(forward_hook)
        output = _run_forward(forward_fn, inputs, target_ind, additional_forward_args)
        assert len(output.shape) < 2 or (len(output.shape) == 2 and len(output[0]) == 1), "Target not provided when necessary, cannot"
