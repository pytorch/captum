...
class LLMAttribution(Attribution):
    ...
    def _forward_func(
        ...
        # pyre-ignore[9] pyre/mypy thinks sum returns int here, but it will return Tensor
    ...
    def _format_model_input(self, model_input: Union[str, Tensor]) -> Tensor:
        ...
        if isinstance(model_input, str):
            # pyre-ignore[9] pyre/mypy thinks return type may be List, but it will be Tensor
            return self.tokenizer.encode(model_input, return_tensors="pt").to(  # type: ignore
                self.device
            )
        ...
        **kwargs: Any,
        ...
        if target is None:
            ...
            if type(target) is str:
                ...
                raise TypeError(
                    "target must either be str or Tensor, but the type of target is {}".format(
                        type(target)
                    )
                )
